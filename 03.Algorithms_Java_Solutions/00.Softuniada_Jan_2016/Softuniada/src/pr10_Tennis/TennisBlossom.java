 ...
 * Code taken from SoftUni GitHub
 */
public class TennisBlossom {
    ...
    public static void main(String[] args) {
        ...
        List<List<Integer>> graph = new ArrayList<>(people.size());
        for (int i = 0; i < people.size(); i++)
        {
        ...
        while (qh < qt) {
            ...
            for (int to : graph.get(v)) {
                ...
                if (to == root || match[to] != -1 && p[match[to]] != -1) {
                    int currBase = lca(match, base, p, v, to);
                    boolean[] blossom = new boolean[n];
                    markPath(match, base, blossom, p, v, currBase, to);
                    markPath(match, base, blossom, p, to, currBase, v);
                    for (int i = 0; i < n; ++i)
                        if (blossom[base[i]]) {
                            base[i] = currBase;
                            if (!used[i]) {
