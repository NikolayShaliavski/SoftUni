...
public class RepositoryServiceBuilder {
    ...
    private static void createRepository(Map<String, Class> entityClassesMap, String repositorypostfix) {
        ...
        for (Map.Entry<String, Class> classEntry : entityClassesMap.entrySet()) {
            ...
            createService(currentClass, importEntityPackage);
            createServiceImplementations(currentClass, importEntityPackage);
        }
    }

    private static void createService(Class currentClass, String importEntityPackage) {
        ...
        if (!serviceFile.exists()) {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(serviceFile, true))) {
                ...
                builder.append(System.lineSeparator());
                builder.append(System.lineSeparator());
                Character classNameFirstLetter = className.charAt(0);
                builder.append(String.format(
                        Constants.SERVICE_INTERFACE_METHOD, className,
                        Character.toLowerCase(classNameFirstLetter) + className.substring(1)));
                builder.append(System.lineSeparator());
                builder.append(Constants.CLOSE_BRACKET);
                writer.write(builder.toString());
                writer.flush();
    ...
    private static void createServiceImplementations(Class currentClass, String importEntityPackage) {
        String className = currentClass.getSimpleName();
        ...
        if (!serviceImplFile.exists()) {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(serviceImplFile, true))) {
                ...
                builder.append(System.lineSeparator());
                builder.append(System.lineSeparator());
                builder.append(Constants.OVERRIDE_ANNOTATION);
                builder.append(System.lineSeparator());
                Character classNameFirstLetter = className.charAt(0);
                String classVariableName = Character.toLowerCase(classNameFirstLetter) + className.substring(1);
                builder.append(String.format(
                        Constants.SERVICE_METHOD, className, classVariableName));
                builder.append(System.lineSeparator());
                builder.append(System.lineSeparator());
                builder.append(String.format(
                        Constants.FIELD_INVOCATION, repoVariableName, classVariableName));
                builder.append(System.lineSeparator());
                builder.append(Constants.INDENTED_CLOSE_BRACKET);
                builder.append(System.lineSeparator());
                builder.append(Constants.CLOSE_BRACKET);
                writer.write(builder.toString());
                writer.flush();
