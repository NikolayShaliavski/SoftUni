...
//written in 9 LAB -> functionality to download file from internet using multi-threading
public class DownloadManager {
    ...
    public static void downloadOnNewThread(String fileUrl) {
        Thread thread = new Thread(() -> download(fileUrl));//the most popular way to create new Thread today -> with lambda expression
        OutputWriter.writeMessageOnNewLine(
                String.format("Worker thread %d started download..", thread.getId()));
        thread.setDaemon(false);//if thread.setDaemon is true our new thread will be executed by main thread, but we don't want this
        thread.start();
    }
