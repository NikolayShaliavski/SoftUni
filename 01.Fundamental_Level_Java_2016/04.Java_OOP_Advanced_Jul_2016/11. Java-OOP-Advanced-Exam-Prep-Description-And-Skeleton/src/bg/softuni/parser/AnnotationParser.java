...
import bg.softuni.lifecycle.ComponentScan;
import bg.softuni.lifecycle.RequestMapping;
import bg.softuni.lifecycle.component.Component;
import bg.softuni.lifecycle.component.Inject;
import bg.softuni.lifecycle.controller.Controller;
import bg.softuni.lifecycle.controller.ControllerActionPair;
import bg.softuni.lifecycle.controller.UriParameter;
...
public class AnnotationParser implements Parser {
                    ...
                    for (Method currentMethod : currentClass.getDeclaredMethods()) {
                        if (currentMethod.isAnnotationPresent(RequestMapping.class)) {
                                ...
                                if (mappingTokens.get(i).startsWith("{") && mappingTokens.get(i).endsWith("}")) {
                                    for (Parameter param : currentMethod.getParameters()) {
                                        ...
                                        if (mappingTokens.get(i).equals("{" + uriParameter.value() + "}")) {
                                            ...
                                            mapping = mapping.replace(mappingTokens.get(i), param.getType() == String
                                                    .class ? "\\w+" : "\\d+");
                                            break;
                                        }
                                    }
                            ...
                            ControllerActionPair pair = new ControllerActionPair(
                                    controllerInstance, currentMethod, argumentsMapping);
    ...
    private Iterator<Class> getDeclaredClasses() throws ReflectiveOperationException {
