...
public class HashTable<TKey, TValue> : IEnumerable<KeyValue<TKey, TValue>>
{
    ...
    public TValue this[TKey key]
    {
        ...
        set
        {
            //set new value using indexer in C#
    ...
    {
        var slotToRemove = this.FindSlotNumber(key);
        var elements = this.slots[slotToRemove];
        //if (elements == null)
        //{
        //    return false;
        //}
    ...
    private void Grow()
    {
        var growedSlots = new LinkedList<KeyValue<TKey, TValue>>[this.Capacity * 2];
        foreach (var elements in this.slots)
        {
            if (elements != null)
            {
                foreach (var element in elements)
                {
                    var newSlotNumber = this.FindSlotNumber(element.Key);
                    if (growedSlots[newSlotNumber] == null)
                    {
                        growedSlots[newSlotNumber] = new LinkedList<KeyValue<TKey, TValue>>();
                    }
                    growedSlots[newSlotNumber].AddLast(element);
                }
            }
        }
        this.slots = growedSlots;
    }
}
